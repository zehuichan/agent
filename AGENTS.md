...
- **提交时机由维护者控制**。AI 助手**永远不主动发起** `git add` / `git commit` / `git push` / `git merge` / `git rebase` / `git tag` / `git reset`。
  哪怕本地 `typecheck + lint + build` 全绿、哪怕"就差临门一脚"，也**必须等维护者显式说"提交"**才执行。
- **一个 commit = 一个 atomic case/feat**。禁止把多个无关改动合进同一 commit，即便每个都只有一行。
  例子：
  - ✅ `feat(chat): add message bubble component`
  - ✅ `fix(stream): handle premature SSE close`
  - ❌ `chore: add bubble + fix stream + tweak readme`（三件事 → 三个 commit）
- 改动涉及**多个 atomic 单元**时，先**在对话里列出拆分方案**（N 个 commit、每条 scope + subject），由维护者确认后再逐条 `git add <文件子集>` + `git commit`。
- `style: prettier baseline` / `chore: rename file` / `docs: ...` 这种纯粹非功能改动**独立成 commit**，不与 `feat/fix` 捆绑。
- Commit 信息一律走 Conventional Commits：`<type>(<scope>): <subject>`，body 写**为什么**不写**做了什么**。
...
- **精简明了，贴合语义场景**：一个函数只做一件事，名字即文档。
- 读者扫一眼签名（名字 + 入参 + 返回类型）就该猜到它在干嘛，不需要翻实现。
- 超过 ~40 行或出现 3 层以上嵌套 → 拆。参数超过 4 个 → 合成 options 对象。
- 命名用动词短语贴业务语义：`parseSseChunk` / `buildToolPayload` / `shouldRetry`，而不是 `handle` / `process` / `doStuff` 这种空壳。
- 纯函数优先；有副作用的函数名字要让副作用显形（`persistXxx` / `emitXxx` / `abortXxx`）。
- 不为"复用可能性"提前抽象，出现**第二个**调用点再抽。
...
- 不擅改技术栈 / 加依赖 / 删文件。
- **提交时机由维护者控制**：不主动 `git add` / `git commit` / `git push` / `git merge` / `git rebase` / `git tag` / `git reset` / `git checkout -b`。
- **一个 commit 只做一件事**：不把 format + feat + fix + docs 捆绑。多单元改动必须拆 commit，拆分方案先给维护者确认。
