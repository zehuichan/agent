...
pi、dsh 的目录可以对照 [pi](https://github.com/earendil-works/pi)、[dsh](https://github.com/deepseek-ai/deepseek-harness) 仓库逐行核对。其余参考系是归纳出来的骨架。

类型由产品定，不由本仓预选。怎么拿需求文档判型见文末；已判型的产品一产品一篇，在 `products/`。
...
- **没有客户。** 通用运行时卖给开发者靠生态，生态靠契约稳定；契约变一次，扩展死一批。本仓今天的 `why-not-pi.md` 答案就是这一条：没有开发者客户，不做通用运行时；`core` 只作各产品共用的库（见「跨类型不变的部分」）；要编码 Agent 就用 Claude Code / Cursor / pi，不自研。出现开发者客户或第二宿主再回来建 `protocol` / `server` / `client` / `extension-api`。
...
这一节原来写的是「本仓在垂直类和通用类之间选，不做另外三类：没有现成产品可以外挂」。撤回。第一个进来的产品恰好落在「不做」的那一类——错的不是分型，是**替仓预选了型**。类型是产品的属性，不是仓的属性；仓只管两件事：判型的标尺，和每一型的骨架。哪一型，由产品需求文档定。
...
`core` 作为被多个产品依赖的库，五类共守（借 Claude Code「框架最薄、复杂度交给模型」的取向，落成生产约束）：

1. `core` 无领域词；CI grep 守门，「线索」「合同」「工单」这类词不许出现。
2. tool 必填副作用声明（读 / 写 / 不可逆）；不可逆默认等确认，确认走 session input；聊天 / 语音里的「好的」「嗯」不算授权。
3. 三上限（步数 / token / 时间）由宿主注入，缺配置不启动；停止原因显式落 entry：`done` / `step_cap` / `token_cap` / `timeout` / `killed` / `needs_confirm`。
4. `compaction` 有回归集：压缩后「保留事实集」全部可在摘要中检索到；每 loop 记 token。
5. prod 无自改 prompt；prompt 来自注册的版本。进化决策给人：知识 / prompt 走版本与审核。
6. 通用五件（bash / read / write / edit / fetch）只作可选子路径导出；只有编码类宿主装配 bash，面向业务方的产品不装——攻击面，不是能力。
7. `core` 变更先过录制快照 eval：固定工具返回序列 → loop 产出同一工具调用序列，并在第 N 步触发 `step_cap`。产品仓钉版本升级。
8. 每 loop 一条 telemetry：步数、token、时延、工具调用、停止原因、prompt 版本、模型版本。`core` 没有自己的值班，它的告警出现在各产品看板上。
...
一产品一篇，在 `products/`，清单见 `products/README.md`。每篇按本文的骨架判型、填目录、定硬规则，再补上线运营要求。本文只写类型，不写具体产品。

### 强制收敛的检验

**第一条 eval 写什么。**「合同到期前 7 天正确创建跟进任务」是垂直类；「在陌生仓库里改对一个函数」是通用类；「一句『把张三标成高意向』落成一次正确的 `contacts.update`」是助手类。一个产品只有一个第一条；写出两条不同类的，就是两个产品。
