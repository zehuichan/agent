...
本仓在**垂直类**和**通用类**之间选，见文末。

---

## 怎么读

每一类固定几节：命题与参考系、产品简笔画、流程图、目录、层 → 目录、硬规则、失败模式；本仓要做的两类多一节完成线。看完五类看「对照」，最后看「本仓怎么选」。
...
左边是完整产品，右边是外挂。把右边整个删掉，左边不缺任何功能——这是助手类的定义。
...
| 领域 | `api/modules/contacts` `deals`       | 不知道聊天存在；权限在这里                   |
...
1. tools 只经领域 service，复用同一套权限校验；不直连表、不拼 SQL。
...
结构特征（用来识别，不是优点）：没有独立 agent 进程；没有 `AgentTask` / `claimDue`；prompt / tool 挂在 `chat` 模块，和 `contacts` 平级，不拥有调度；供应商 SDK 出现在 API 或 Next route；记忆往往只有 `messages[]`。
...
在**垂直类**和**通用类**之间选，不做另外三类：没有现成产品可以外挂；目标不是固定流程；多 Agent 是组织方式，等单 agent 做不到再拆。

### 两类共用的骨架

`telemetry` / `ai` / `core` / `session-backends` / `queue` 五个包两边一样，`core` 在两边都无领域。差别是 `<domain>` + `integrations` + `web` 存在不存在、`protocol` / `server` / `client` / `extension-api` 是 P0 还是 P3、`TaskSpec` 是 `{ type, payload }` 还是 `{ goal, cwd?, tools? }`。所以骨架可以先起，方向可以晚一点定——但不能不定。

共用的接法：任务是**意图**，会话是**执行记录**。认领一个到期任务就开一个 session——调度开的会话和人开的会话是同一种东西，同一条事件流，同一个查看器。
...
共用的规则：可插点三角色（契约 / 实现 / 消费者，conformance 挂契约包，少一角就只是内部实现）；模型可见 ⟺ 已记录；HTTP 是薄壳；`queue` 不 import `core`；无 `examples/`，验证靠 test / eval。
...
**第一条 eval 写什么。**「合同到期前 7 天正确创建跟进任务」是垂直类；「在陌生仓库里改对一个函数」是通用类。不选就是两头都做：通用骨架永远没有客户，垂直产品永远不干净。
